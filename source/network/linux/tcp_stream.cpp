...
bool tcp_stream::fill_addr(std::string const &str_addr, uint16_t port,
                           sockaddr_in &addr) {
  addr = {0, 0, {0}, {0}};
...
bool tcp_stream::connect_to_server(std::string const &peer_addr,
...
bool tcp_stream::bind_as_server(std::string const &peer_addr,
                                uint16_t peer_port, struct ev_loop *loop,
                                proccess_incoming_conn incom_con,
                                std::any asoc_data) {
  bool res{false};
  if (!fill_addr(peer_addr, peer_port, _self_addr)) return res;
  _incom_con = incom_con;
  _incom_con_data = asoc_data;
...
ssize_t tcp_stream::send_data(std::byte *data, size_t data_size) {
  ssize_t sent{0};
  while (true) {
...
ssize_t tcp_stream::get_data(std::byte *buffer, size_t buffer_size) {
  ssize_t rec{0};
  while (true) {
...
void tcp_stream::set_received_data_cb(received_data_t cb, std::any user_data) {
  _received_data = cb;
  _received_user_data = user_data;
}

void tcp_stream::set_send_data_cb(send_data_t cb, std::any user_data) {
  _send_data = cb;
  _send_user_data = user_data;
}

void tcp_stream::set_state_changed_cb(state_changed_t cb, std::any user_data) {
  _state_changed = cb;
...
void tcp_stream::send_data() {
  if (_send_data) _send_data(this, _send_user_data);
}
...
void tcp_stream::handle_incoming_connection(int file_descr,
  ...
  if (_incom_con) _incom_con(std::move(sck), _incom_con_data);
}
